...
generator.push({name:NameGen.compile("!BsV'i").toString()});//items
generator.push({name:NameGen.compile("!BsV !BsV").toString()});//pnj
generator.push({name:NameGen.compile("bss sV").toString()});//monstre
generator.push({name:NameGen.compile("vvd css").toString()});//monstre
generator.push({name:NameGen.compile("css vvd").toString()});//pnj
}

var data = JSON.stringify(generator, null, 2);
fs.writeFile('name.json', JSON.minify(data), (err) => {
  if (err) throw err;
});